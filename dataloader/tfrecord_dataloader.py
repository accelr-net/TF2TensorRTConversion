...
class TfRecorder(Data):
    ...
    def preprocessing_res50_hook(self, image: tf.Tensor, height: int, width: int) -> tf.Tensor:

        """
        Preprocess the image by normalizing and resizing it according to the specifications need by resnet50v1 model.
    ...
    def preprocess(self, record: tf.Tensor) -> tuple:
        ...
        image = self.preprocessing_res50_hook(image, 224, 224)
